...
chcp 65001 >nul
title Web Security Scanner v8.5 (Explicit Result)

:MAIN_MENU
cls
echo.
echo ========================================================
echo   🛡️ 웹 종합 보안 진단 도구 v8.5 (상세 결과판)
echo ========================================================
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$ua = 'Mozilla/5.0 (compatible; SecurityCheck/8.5)'; "^
    ""^
    "try { "^
    ...
    "               }; "^
    "           } catch { "^
    "               Write-Host ' ✅ 안전 (Shodan 미탐지)' -ForegroundColor Green; "^
    "           } "^
    "       } "^
    "    }; "^
    ""^
    ...
    "        } "^
    "    } "^
    "} catch { Write-Host '   [!] 접속 실패. 주소를 확인하세요.' -ForegroundColor Red; exit }"^
    ""^
    "Write-Host ''; "^
    ...
    "        if($errCode){ Write-Host ('     Result: ' + $errCode + ' (🛡️ 차단됨 - 안전)') -ForegroundColor Cyan } "^
    "        else { Write-Host ('     Result: 연결 오류') -ForegroundColor DarkGray } "^
    "    } "^
